<?php 
	...
	*/
	class Retrieve_password extends CI_Controller
	{
		...
		{
			$temp['title'] = "Reset mật khẩu"; 
			$temp['message'] = '';
			$email = $this->input->post('email');
			if($email != '')
			{
				$this->load->model('blogging_model');
				if(!$this->blogging_model->check_email($email))
				{
					$temp_pass = substr(md5(uniqid(rand(), true)), 3, 10);
					if($this->blogging_model->reset_password($email,$temp_pass) == 1)
					{
						
						$body = "Bạn vửa reset mật khẩu: \n\n ";
	            		$body.="Email: {$email}\nPassword: {$temp_pass}";
						if($this->mail_to($email,'Reset mật khẩu FAbook',$body))
						{
							$temp['message'] = "<p class=\"message\">Email reset mật khẩu đã được gửi đi. 
						Xin vui lòng kiểm tra email của bạn để lấy lại mật khẩu</p>";
						}
						else
						{
							$temp['message'] = "<p class=\"message\">Chúng tôi không thể gửi email reset mật khẩu đến email của 
							bạn. Xin lỗi vì sự bất tiện này. </p>";
						}
					}
					else
					{
						$temp['message'] = "<p class=\"warning\">Reset mật khẩu thất bại. Xin vui lòng thử lại</p>";
					}
				}
				else
				{
					$temp['message'] = "<p class=\"warning\">Không tồn tại email này trong hệ thống</p>";
				}
			}
			$this->load->view("blogging/vretrieve_password",$temp);
		}
		private function mail_to($mail_address,$subject,$body)
		{
