<?php 
	...
	*/
	class Register extends CI_Controller
	{
		...
		{		
			$temp['title'] = "Đăng kí tài khoản"; 
			$temp['message'] = '';
			$this->load->helper('form');
			$username = $this->input->post('username');
			$password = $this->input->post('password');
			$passconfirm = $this->input->post('passwordconfirm');
			$email = $this->input->post('email');
			$this->load->model('blogging_model');
			if($username != '' && $password != '' && $passconfirm != '' && $email != '')
			{
				if($this->blogging_model->check_username($username) && $this->blogging_model->check_email($email))
				{
					$a = md5(uniqid(rand(),true));
					$this->blogging_model->insert_user($username, $password, $email, $a);
					$body = "Cảm ơn bạn đã đăng ký tài khoản tại FAbook. 
                Xin vui lòng click vào đường link bên dưới để kích hoạt tài khoản \n\n ";
            		$body.= base_url() . "index.php/activate/result/".urlencode($email)."/{$a}";
					if($this->mail_to($email,'Kích hoạt tài khoản tại FAbook',$body))
					{
						$temp['message'] = "<p class=\"message\">Tài khoản đăng kí thành công.
					...
					{
						$temp['message'] = "<p class=\"message\">Chúng tôi không thể gửi email kích 
						hoạt đến email của bạn. Xin lỗi vì sự bất tiện này. </p>";
					}
					$this->load->view("blogging/vregister",$temp);
				}
